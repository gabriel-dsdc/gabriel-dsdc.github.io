...
function Projects({ repos, reposExtras, isFetching }) {
  ...
  return (
    <>
      {isFetching && <h1 className="text-center text-2xl"><TranslatedText id="loading" />...</h1>}
      <h1 id="projects" className="text-center font-bold mb-2"><TranslatedText id="projects" /></h1>
      <form className="mb-1 text-center">
        <select className="mr-1" onChange={handleType} value={type}>
          <option value=""><TranslatedText id="selectType" /></option>
          <option value="Front-end">Front-End</option>
          <option value="Back-end">Back-End</option>
        </select>
        <select onChange={handleStack} value={stack}>
          <option defaultValue value=""><TranslatedText id="selectStack" /></option>
          {Object.entries(stacks).map(stack => (
            <option key={stack[0]} value={stack[0]}>{stack[1].name}</option>
          ))}
        </select>
      </form>
