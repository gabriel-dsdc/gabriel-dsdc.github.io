...
function Header({ languageToggle, currentLang }) {
  ...
  return (
    <header className="flex justify-between">
      <button
        type="button"
        className="after:content-[attr(content)] after:absolute after:block after:ml-2"
        content={`${currentLang !== "en" ? "Language" : "Idioma"}`}
        onClick={languageToggle}
      >
        <img
          className="w-10 h-10 self-center ml-3 mt-3 cursor-pointer"
          src={currentLang === "en" ? enSelected : ptSelected}
          alt="language flag"
        />
      </button>
      <nav className="w-full">
        <ul className="h-full flex justify-center items-center">
          <li className="font-bold">
            <NavLink
              to="#home"
              className="p-4 dark:hover:bg-neutral-700 hover:bg-zinc-200"
            >
      ...
      <label
        ...
        className="h-min pt-3 pr-3 after:content-[attr(data-theme)] after:absolute after:block after:cursor-pointer"
      >
